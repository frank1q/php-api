...
class photoModel extends baseModel{
    ...
    public function photoANDextend($exttables=array(),$path='',$limit='',$id,$exwhere,$keyword=''){
        ...
        $sql="SELECT {$this->prefix}photo.id,{$this->prefix}photo.title,{$this->prefix}photo.color,{$this->prefix}photo.sort,{$this->prefix}photo.exsort,{$this->prefix}photo.picture,{$this->prefix}photo.hits,{$this->prefix}photo.addtime,{$this->prefix}photo.method,{$this->prefix}photo.description{$exfield} FROM {$this->prefix}photo left outer join {$this->prefix}{$exttable} on ({$this->prefix}photo.extfield = {$this->prefix}{$exttable}.id) where  {$where}  ORDER BY {$this->prefix}photo.recmd DESC,{$this->prefix}photo.norder desc,{$this->prefix}photo.id DESC LIMIT {$limit}";

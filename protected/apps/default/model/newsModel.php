...
class newsModel extends baseModel{
	...
	public function newsANDextend($exttables=array(),$path='',$limit='',$id,$exwhere='',$keyword=''){
		...
		$sql="SELECT {$this->prefix}news.id,{$this->prefix}news.title,{$this->prefix}news.color,{$this->prefix}news.sort,{$this->prefix}news.exsort,{$this->prefix}news.picture,{$this->prefix}news.origin,{$this->prefix}news.hits,{$this->prefix}news.addtime,{$this->prefix}news.method,{$this->prefix}news.description{$exfield} FROM {$this->prefix}news left outer join {$this->prefix}{$exttable} on ({$this->prefix}news.extfield = {$this->prefix}{$exttable}.id) where  {$where}  ORDER BY {$this->prefix}news.recmd DESC,{$this->prefix}news.norder desc,{$this->prefix}news.id DESC LIMIT {$limit}";

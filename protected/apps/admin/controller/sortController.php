...
class sortController extends commonController
{
   ...
   //封面图剪切
            ...
            if(empty($_POST['ifnums'])){
               ...
               if(!empty($sortnames)){
               	  ...
               	  foreach ($sortnames as $vo) {
               	  	 ...
               	  	 if(!empty($vo)){
               	  	 	$data['name']=$vo;
               	  	 	$data['ename']=$maxid+$i+1;
               	  	 	if(!model('sort')->insert($data)) $this->error('资讯栏目"'.$vo.'"添加失败~');
               	  	 	else $i++;
               	  	 }
               	  }
	...
	public function newsedit()
	{
		...
		$info=model('sort')->find("id='$id'",'name,ename,norder,path,picture,ifmenu,url,method,tplist,keywords,description,extendid');
               ...
               if(!empty($sortnames)){
               	  ...
               	  foreach ($sortnames as $vo) {
               	  	 ...
               	  	 if(!empty($vo)){
               	  	 	$data['name']=$vo;
               	  	 	$data['ename']=$maxid+$i+1;
               	  	 	if(!model('sort')->insert($data)) $this->error('图集栏目"'.$vo.'"添加失败~');
               	  	 	else $i++;
               	  	 }
               	  }
	...
	public function photoedit()
	{
		...
		$info=model('sort')->find("id='$id'",'name,ename,norder,path,ifmenu,picture,url,method,tplist,keywords,description,extendid');
		$info['url']=empty($info['url'])?10:$info['url'];
		$oldparentid=intval(substr ($info['path'], -6));
		$tps=explode(',',$info['tplist']);
		$info['tplist']=$tps[0];

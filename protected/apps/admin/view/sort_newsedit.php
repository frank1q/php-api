...
function preview(img, selection) { //剪切区域改变触发函数
  ...
  $(function ($) { 
	...
	$("#cut").click(function(){//剪切图片
		...
		$.post("{url('sort/cutcover')}", { name: name, x1: x1, y1: y1, w: w, h: h,thumb_w: thumb_w,file:'news/image'}, 
			   function(data){
				   if(data){
					$("#coverimg").hide();
...
<div class="list_head_m">
         ...
         <table width="100%" border="0" cellpadding="0" cellspacing="1"   class="all_cont">
          <form enctype="multipart/form-data" action="{url('sort/newsedit',array('id'=>$id))}"  method="post" id="info">
          <tr>
